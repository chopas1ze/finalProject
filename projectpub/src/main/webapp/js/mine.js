...
function joinpubok(){
	...
	if($('#jppub_contents').val()==''){
		alert("펍 소개글을 등록해주세요");
		$('#jppub_contents').focus();
		return false;
	}
	...
	if($('#jppub_maxpeople').val()==''){
		alert("펍 주소를 등록해주세요");
		$('#jppub_maxpeople').focus();
		return false;
	}
	
	
	if($('#jppub_mupload')[0].files.length == 0){
		alert('펍 사진이 최소 1개이상 등록되어야 합니다');
		return false;
...
function uptpubok(){
	...
	if($('#uppub_contents').val()==''){
		alert("펍 소개글을 등록해주세요");
		$('#uppub_contents').focus();
		return false;
	}
	
	if($('#uppub_maxpeople').val()==''){
		alert("예약 가능 최대 인원수를 적어주세요");
		$('#uppub_maxpeople').focus();
		return false;
	}
	
	if($('#uppub_mupload')[0].files.length == 0){
		alert("펍 소개 사진이 최소 1장은 등록되어야 합니다");
		return false;
