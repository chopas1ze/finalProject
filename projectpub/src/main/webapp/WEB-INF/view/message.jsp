...
<body>
	...
	<div class="full">
		...
		<div class="mainmenu">
			...
			<div class="head_ment">
				<c:choose>
					<c:when test="${chkmsg!=null}">
						<span>${sessionScope.mem.id}님의 받은 쪽지함입니다.(읽지 않은
							쪽지:${chkmsg}통)</span>
					</c:when>
					<c:otherwise>
						<span>${sessionScope.mem.id}님의 받은 쪽지함입니다.(읽지 않은 쪽지:0통)</span>
					</c:otherwise>
				</c:choose>
			...
			<div class="head_button">
				...
				<button id="delete_btn" class="btn btn-default btn-sm">삭제</button>
									...
									<c:choose>
										...
										<c:otherwise>
											<tr>
												...
												<td  width="270px" class="text-center"><img src="images/newicon.png" style="width: 20px; height: 20px;"><a
													href="${viewPage}">${rm.t_title}</a></td>
												<td  width="110px" class="text-center">${rm.id}</td>
												<td  width="190px" class="text-center">${fn:substringAfter(rm.t_date,"-")}</td>
											</tr>
				...
				<div class="search_div">
					<form id="mesgfrm" method="get" action="searchmsg.do">
						<select id="searchop" name="searchop">
							...
							type="button" id="search_btn" value="검색" class="btn btn-success btn-sm" /> <input type="hidden"
							name="id" value="${memid}">
