...
DROP TABLE socialnice CASCADE CONSTRAINTS;
DROP TABLE QUICKBAR CASCADE CONSTRAINTS;
...
DROP SEQUENCE SEQ_socialnice_sn_num;
...
CREATE SEQUENCE SEQ_socialnice_sn_num INCREMENT BY 1 START WITH 1 nocache nocycle;
...
(
	...
	rs_content varchar2(1000) NOT NULL,
	id varchar2(128) NOT NULL,
	PRIMARY KEY (rs_num)
);
...
CREATE TABLE socialnice
(
   s_nice number(10,0),
   id varchar2(128) NOT NULL,
   s_num number(5,0) NOT NULL,
   sn_num number(10,0) NOT NULL,
   PRIMARY KEY (sn_num)
);
...
ALTER TABLE socialnice
   ADD  CONSTRAINT FK_socialnice_id FOREIGN KEY (id)
   REFERENCES members (id) ON DELETE CASCADE
;

ALTER TABLE socialnice
   ADD CONSTRAINT FK_socialnice_num FOREIGN KEY (s_num)
   REFERENCES social (s_num) ON DELETE CASCADE
;

ALTER TABLE QUICKBAR
   ADD  CONSTRAINT FK_quickbar_id FOREIGN KEY (id)
   REFERENCES members (id)  ON DELETE CASCADE
;
ALTER TABLE QUICKBAR
   ADD CONSTRAINT FK_quickbar_num FOREIGN KEY (p_num)
   REFERENCES pub (p_num)  ON DELETE CASCADE
;
