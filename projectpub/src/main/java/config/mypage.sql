...
select * from members
insert into members values('ID','PW','phone','email실제이메일','email_agree=Y','sex=m or f','이름','1950-01-02','userchk=C or S or A','얼굴사진이름확장자까지','allow_chk=Y',to_char(sysdate,'YYYY"년"MM"월"DD"일"'),null,to_char(sysdate,'YYYY"-"MM"-"DD'))
...
res_sellchk=N 이면 예약신청(7), C 예약 거부(3), Y예약 성공(5명,성공한 사람들에게는 쿠폰테이블의 구매자ID에 해당하는 시퀀스 삽입)  수락인원-> 거부-> 예약신청순으로 삽입
insert into reservation values(SEQ_reservation_res_num.nextval,펍시퀀스넘버,'구매자ID','예약인원수',to_char(sysdate,'YYYY"-"MM"-"DD'),'res_concheck=Y','res_sellchk=N or C or Y',to_char(sysdate,'HH"시'),구매자쿠폰시퀀스(or NULL));
...
update members set userchk='S' where id='rudghekt';
update members set userchk='A' where id='administer';
update members set allow_chk='N' where id='cus';
update members set allow_chk='R' where id='sell2';
update members set allow_chk='Y' where id='rudghekt';
update members set email_agree='Y' where id='selltest2';
...
select * from pub
delete from pub 
update pub set p_pub_chk='Y' where id='rudghekt'
 

--reservation 등록
insert into reservation values(SEQ_reservation_res_num.nextval,1,'customer','10',to_char(sysdate,'YYYY"년"MM"월"DD"일"'),'Y','N',to_char(sysdate,'HH"시'),2);
...
from(select p.*, r.res_num,r.p_num as rpnum , r.id as rrid, r.res_people,r.res_date,r.res_concheck,r.res_sellcheck,r.res_time,r.c_num,m.name,c.c_use,c.c_contents
from(select * from pub where id='seller01')p,  reservation r, members m, coupon c
where p.p_num=r.p_num(+) and  r.id=m.id(+) and r.c_num=c.c_num(+)
order by r.res_num desc)a)b
where b.rm<=11
...
update talk set t_check = 'Y' where t_num=6

--읽지 않은 쪽지 갯수
select count(*) from(select * from talk where t_resp='customer' and t_check='N')

--읽은 쪽지 check값 변경
update talk set t_check='Y' where t_num=3

 select count(*) as count from(select * from talk where t_resp='customer' and t_check='N')
 
 select * from talk where t_resp='customer' and t_check='N'
 
 --퀵바 테이블 삭제(로그아웃)
 delete from quickbar where id='customer'
 ...
 create or replace procedure delcoupon_job
 is begin
 delete from coupon where c_date=to_char(sysdate,'YYYY"년"MM"월"DD"일"')
 commit;
 end;
 /
...
job_name => 'coupon_job' ,
 ...
 job_action => 'BEGIN delcoupon_job; END;' ,
 ...
 repeat_interval =>'FREQ=DAILY;INTERVAL=1' );
  ...
  exec dbms_scheduler.enable('delcoupon_job');
  
  exec dbms_scheduler.run_job('delcoupon_job');
